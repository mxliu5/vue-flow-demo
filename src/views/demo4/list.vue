<template>
            ...
            <tu-query
              ...
              <template #content="{item}">
                <el-input
                  ...
                  <template #append>
                    <el-button :icon="Search" @click="searchQuery"/>
                  </template>
                  </el-input>
              </template>
            </tu-query>
  ...
  const emits = defineEmits(['queryCacheInit'])
  const props = defineProps({
    activeType: { type: String }
  })
  
  // hooks
  const routeName = 'mock-demo4'
  const { $app, proxy, $router } = useCtx();
  const { query, queryItems, searchQuery, selectedIds, handleSelectionChange } = useTablePage(routeName, emits);
